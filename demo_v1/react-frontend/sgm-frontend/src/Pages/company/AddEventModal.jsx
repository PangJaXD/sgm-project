        ...
        <div className="p-8 text-[13px] text-gray-800">
          ...
          <div className="mb-6">
            <h3 className="font-bold text-[14px] text-center mb-4">
              ช่วงเวลาปฏิบัติงานและเจ้าหน้าที่ (รวม{" "}
              {eventData.required_guards || 0} คน)
            </h3>
            {sortedShifts.length === 0 ? (
              ...
              <div className="flex flex-col gap-4 w-full max-w-[600px] mx-auto">
                {sortedShifts.map((shift, idx) => {
                  ...
                  // 🌟 รองรับทั้ง Object headGuard และ ID เดิม
                  const headGuardName = shift.headGuard
                    ? `${shift.headGuard.first_name || ""} ${shift.headGuard.last_name || ""}`.trim()
                    : "ยังไม่ระบุหัวหน้า";
                  ...
                  return (
                    ...
                    >
                      <p className="text-[#2864e8] font-semibold mb-3">
                        กะที่ {idx + 1} : วันที่{" "}
                        {shift.start_time?.split("T")[0] || startDate}
                      </p>
                      <div className="grid grid-cols-2 gap-4">
                        <div className="flex items-center justify-between bg-white px-3 py-2 rounded-lg border border-gray-100 col-span-2">
